...
class ValidationController
{
    ...
    public function checkResa($post)
    {
        $errors = [];
        $datas = [];
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if (!isset($_POST['user_lastname']) || empty($_POST['user_lastname'])) {
                $errors['lastname'] = 'Veuillez renseigner votre champs nom';
            ...
            } elseif (!preg_match("/^([a-zA-Z' ]+)$/", $_POST['user_mail'])) {
            ...
            } elseif (!preg_match("/^([a-zA-Z' ]+)$/", $_POST['user_phone'])) {
            ...
            if (!isset($_POST['user_adress']) || empty($_POST['user_adress'])) {
                $errors[] = 'Veuillez renseigner le champs adresse';
            } elseif (!preg_match("/^([a-zA-Z' ]+)$/", $_POST['user_adress'])) {
                $errors[] = 'Veuillez saisir une adresse valide';
            } else {
                $datas[] = $this->testInput($_POST['user_adress']);
            }

            if (!isset($_POST['user_zip']) || empty($_POST['user_zip'])) {
                $errors['zip'] = 'Veuillez renseigner le code postal';
            } elseif (!preg_match("/^([a-zA-Z' ]+)$/", $_POST['user_zip'])) {
